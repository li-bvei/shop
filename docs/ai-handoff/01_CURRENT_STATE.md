...
- 当前 HEAD：`c58287c dailyreports: lock reports dated before today behind a shared unlock password`
- 上一个重要提交：`1453cc5`（收银机零钱默认数量 + レジ固定金額可改）→ `1fc5fa1`（批量替换空字符串/日期校验 bug 修复）→ `960bbac`（进货批量替换重构为多条件组合 + 2026注文書 数据核对）。
...
- 新增"収银机现有金额"零钱默认数量：500/100/50/10/5 五档面额可按分店设置一个默认枚数（后端 `CashRegisterDefaults` 模型，一分店一条记录，`denomination_defaults` JSON 字段）。**不会**预填进"枚数"输入框——枚数始终是当天实际数出来的数量，默认值只在计算小計/合计时自动加算（`computeCashRegisterTotal`/`cashRegisterRowSubtotal`），且只对**本次真的填了数字**的行生效，从未填写过的行永远是 ¥0，避免把默认值套到"从来没记录过収银机数据"的历史日报上。为兼容 2026-09-15 之前保存的旧日报（当时这个功能还不存在，枚数字段的含义是"全额"而不是"额外数量"），额外加了硬编码日期线 `CASH_REGISTER_DEFAULTS_CUTOFF_DATE='2026-09-15'`：日报日期早于这条线的，默认值永远不生效，不管该行有没有填数字。同一处レジ固定金額（原来硬编码 ¥130,000）可编辑，编辑后同样只影响新报表和 Excel 导出。仅主日报表单开放编辑默认值（`allow-cash-register-default-edits` prop），历史记录编辑弹窗保持只读展示，但两处都会按同一套日期线+空值规则计算小計。
