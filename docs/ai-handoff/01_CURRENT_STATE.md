...
- 新增"収银机现有金额"零钱默认数量：500/100/50/10/5 五档面额可按分店设置一个默认枚数（后端 `CashRegisterDefaults` 模型，一分店一条记录，`denomination_defaults` JSON 字段），新建/未填的日报会自动用默认值预填对应面额的"枚数"列，已保存过的历史日报不受影响；改动立刻 PATCH 保存，不需要额外"保存"按钮。同一处新增レジ固定金額（原来硬编码 ¥130,000）可编辑，编辑后同样只影响新报表和 Excel 导出用的预定金额，不改历史记录。仅主日报表单开放编辑（`allow-cash-register-default-edits` prop），历史记录编辑弹窗保持只读展示。
