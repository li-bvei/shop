...
- 当前 HEAD：`829e531 suppliers: filter monthly payable by month; frontend: detect new deploys`
- 上一个重要提交：`c58287c`（旧日报锁定）→ `1453cc5`（收银机零钱默认数量 + レジ固定金額可改）→ `1fc5fa1`（批量替换空字符串/日期校验 bug 修复）→ `960bbac`（进货批量替换重构为多条件组合 + 2026注文書 数据核对）。
