...
import { ArrowDown, ArrowUp, Calendar, Delete, Download, Hide, Plus, Refresh, Search, User, View } from '@element-plus/icons-vue'
...
type DabingView = 'today' | 'browse'
type KyotoView = 'batch' | 'all'
...
/** Plain YYYY-MM-DD arithmetic — the value is already a calendar date, so
 * no timezone conversion is wanted. */
...
function toPickerDate(iso: string): Date {
  const [y, m, d] = iso.split('-').map(Number) as [number, number, number]
  return new Date(y, m - 1, d)
}

// --- recent people: a per-device most-recently-used list, so continuous
// entry is one tap instead of re-searching the same names. ---
interface RecentPerson { id: string; name: string }
const RECENT_MAX = 10
function loadRecent(key: string): RecentPerson[] {
  try { return JSON.parse(localStorage.getItem(key) || '[]') } catch { return [] }
}
function rememberRecent(key: string, store: Ref<RecentPerson[]>, person: RecentPerson) {
  store.value = [{ id: person.id, name: person.name }, ...store.value.filter((p) => p.id !== person.id)].slice(0, RECENT_MAX)
  try { localStorage.setItem(key, JSON.stringify(store.value)) } catch { /* private mode */ }
}
const recentDabing = ref<RecentPerson[]>(loadRecent('lottery_recent_dabing'))
const recentKyoto = ref<RecentPerson[]>(loadRecent('lottery_recent_kyoto'))

// --- dabing ---
const dabingStores = ref<DabingStore[]>([])
const dabingPeople = ref<DabingPerson[]>([])
const dabingRecords = ref<DabingRecord[]>([])
const dabingBrowseRecords = ref<DabingRecord[]>([])
const dabingView = ref<DabingView>((localStorage.getItem('lottery_dabing_view') as DabingView) || 'today')
const dabingDate = ref(todayJst())
const dabingRange = ref<[string, string]>([addDays(todayJst(), -13), todayJst()])
const dabingBrowseKeyword = ref('')
...
const kyotoAllRecords = ref<KyotoRecord[]>([])
const kyotoView = ref<KyotoView>((localStorage.getItem('lottery_kyoto_view') as KyotoView) || 'batch')
const kyotoRange = ref<[string, string]>([addDays(todayJst(), -60), todayJst()])
const kyotoAllKeyword = ref('')
...
const rangeShortcuts = computed(() => [
  { text: t('lottery.rangeLast2w'), value: () => [toPickerDate(addDays(todayJst(), -13)), toPickerDate(todayJst())] },
  { text: t('lottery.rangeThisMonth'), value: () => [toPickerDate(monthStart(todayJst())), toPickerDate(todayJst())] },
  {
    text: t('lottery.rangeLastMonth'),
    value: () => {
      const prevMonthEnd = addDays(monthStart(todayJst()), -1)
      return [toPickerDate(monthStart(prevMonthEnd)), toPickerDate(prevMonthEnd)]
    },
  },
])
...
const recentDabingChips = computed(() => {
  const known = new Map(dabingPeople.value.map((p) => [p.id, p.name]))
  return recentDabing.value.filter((p) => known.has(p.id)).map((p) => ({ id: p.id, name: known.get(p.id) || p.name }))
})
const recentKyotoChips = computed(() => {
  const known = new Map(kyotoPeople.value.map((p) => [p.id, p.name]))
  return recentKyoto.value.filter((p) => known.has(p.id)).map((p) => ({ id: p.id, name: known.get(p.id) || p.name }))
})

const dabingBrowseByDate = computed(() => {
  const map = new Map<string, DabingRecord[]>()
  for (const row of dabingBrowseRecords.value) map.set(row.drawDate, [...(map.get(row.drawDate) || []), row])
  return [...map.entries()].sort((a, b) => (a[0] < b[0] ? 1 : -1)).map(([date, records]) => ({ date, records }))
})
...
const kyotoActiveRecords = computed(() => (kyotoView.value === 'batch' ? kyotoRecords.value : kyotoAllRecords.value))

const groupedKyotoRecords = computed<KyotoWinnerGroup[]>(() => {
  const groups = new Map<string, KyotoRecord[]>()
  for (const record of kyotoActiveRecords.value) {
...
function selectDabingView(view: DabingView) {
  dabingView.value = view
  localStorage.setItem('lottery_dabing_view', view)
  if (view === 'browse') loadDabingBrowse()
}

function selectKyotoView(view: KyotoView) {
  kyotoView.value = view
  localStorage.setItem('lottery_kyoto_view', view)
  if (view === 'all') loadKyotoAll()
}
...
function refreshDabing() {
  if (dabingView.value === 'browse') loadDabingBrowse()
  else loadDabingRecords()
}

function refreshKyoto() {
  if (kyotoView.value === 'all') loadKyotoAll()
  else loadKyotoRecords()
}
...
async function loadDabing() {
  ...
  if (dabingView.value === 'browse') await loadDabingBrowse()
}
...
async function loadDabingBrowse() {
  const [from, to] = dabingRange.value
  if (!from || !to) return
  dabingBrowseRecords.value = await fetchDabingRecords({
    dateFrom: from,
    dateTo: to,
    storeId: dabingOverviewStoreId.value === 'all' ? undefined : dabingOverviewStoreId.value,
    search: dabingBrowseKeyword.value.trim() || undefined,
  })
}
...
async function loadKyoto() {
  ...
  if (kyotoView.value === 'all') await loadKyotoAll()
}

async function loadKyotoRecords() {
  kyotoRecords.value = await fetchKyotoRecords({ batchId: kyotoBatchId.value || undefined })
  expandedKyotoNames.value = {}
}

async function loadKyotoAll() {
  const [from, to] = kyotoRange.value
  kyotoAllRecords.value = await fetchKyotoRecords({
    publishFrom: from || undefined,
    publishTo: to || undefined,
    search: kyotoAllKeyword.value.trim() || undefined,
  })
  expandedKyotoNames.value = {}
}
...
watch(dabingOverviewStoreId, refreshDabing)
...
let dabingBrowseTimer = 0
watch([dabingRange, dabingBrowseKeyword], () => {
  if (dabingView.value !== 'browse') return
  window.clearTimeout(dabingBrowseTimer)
  dabingBrowseTimer = window.setTimeout(loadDabingBrowse, 250)
})
let kyotoAllTimer = 0
watch([kyotoRange, kyotoAllKeyword], () => {
  if (kyotoView.value !== 'all') return
  window.clearTimeout(kyotoAllTimer)
  kyotoAllTimer = window.setTimeout(loadKyotoAll, 250)
})
...
function openPersonDialog(module: Module) {
  ...
  if (module === 'dabing') {
    personForm.mobileModel = localStorage.getItem('lottery_last_mobile_model') ?? ''
    personForm.contact = localStorage.getItem('lottery_last_contact') ?? ''
  }
...
function rememberDabingDefaults() {
  const model = personForm.mobileModel.trim()
  const contact = personForm.contact.trim()
  if (model) localStorage.setItem('lottery_last_mobile_model', model)
  else localStorage.removeItem('lottery_last_mobile_model')
  if (contact) localStorage.setItem('lottery_last_contact', contact)
  else localStorage.removeItem('lottery_last_contact')
}

async function saveDabingRecord(personId?: string) {
  const targetId = personId ?? dabingPersonId.value
  if (!dabingDate.value || !dabingStoreId.value || !targetId) return ElMessage.warning(t('lottery.completeEntry'))
  saving.value = true
  try {
    await createDabingRecord({ storeId: dabingStoreId.value, personId: targetId, drawDate: dabingDate.value, drawTime: dabingTime.value })
    const person = dabingPeople.value.find((p) => p.id === targetId)
    if (person) rememberRecent('lottery_recent_dabing', recentDabing, { id: person.id, name: person.name })
    if (!personId) {
...
async function saveKyotoRecord(personId?: string) {
  const targetId = personId ?? kyotoPersonId.value
  if (!kyotoBatchId.value || !targetId) return ElMessage.warning(t('lottery.completeEntry'))
  saving.value = true
  try {
    await createKyotoRecord({ batchId: kyotoBatchId.value, personId: targetId })
    const person = kyotoPeople.value.find((p) => p.id === targetId)
    if (person) rememberRecent('lottery_recent_kyoto', recentKyoto, { id: person.id, name: person.name })
    if (!personId) {
      kyotoPersonId.value = ''
      kyotoKeyword.value = ''
    }
...
async function exportDabing() {
  ...
  try {
    const browsing = dabingView.value === 'browse'
    const rows = browsing ? dabingBrowseRecords.value : dabingRecords.value
    const tag = browsing ? `${dabingRange.value[0]}~${dabingRange.value[1]}` : dabingDate.value
    await downloadTableExcel(`大饼中奖名单-${tag}`, '大饼中奖名单', [
    ...
    ], rows.map((row) => ({ name: row.personName, phone: row.phone, store: row.storeName, date: row.drawDate, time: row.drawTime, mobile: row.mobileModel })), 'landscape')
  } finally {
...
async function exportKyoto() {
  const all = kyotoView.value === 'all'
  if (!all && !selectedKyotoBatch.value) return
  downloading.value = true
  try {
    const rows = all ? kyotoAllRecords.value : kyotoRecords.value
    const tag = all ? `${kyotoRange.value[0]}~${kyotoRange.value[1]}` : selectedKyotoBatch.value!.displayLabel
    await downloadTableExcel(`京都爱电王中奖名单-${tag}`, '京都爱电王中奖名单', [
    ...
    ], rows.map((row) => ({ name: row.personName, phone: row.phone, period: `${row.drawStartDate}〜${row.drawEndDate}`, publishDate: row.publishDate })))
  } finally {
    downloading.value = false
  }
}
...
<template>
  <div class="lottery-view" v-loading="loading">
    ...
    <template v-if="activeModule === 'dabing'">
      <section class="entry-card card accent-dabing">
        <div class="section-heading"><div><span class="eyebrow">{{ t('lottery.quickEntry') }}</span><h2>{{ t('lottery.dabingEntryTitle') }}</h2></div><el-button text :icon="User" @click="openPersonDialog('dabing')">{{ t('lottery.addPerson') }}</el-button></div>
        <div class="context-grid"><div><label>{{ t('lottery.drawDate') }}</label><el-date-picker v-model="dabingDate" type="date" value-format="YYYY-MM-DD" /></div><div><label>{{ t('lottery.store') }}</label><el-select v-model="dabingStoreId" :placeholder="t('lottery.selectStore')"><el-option v-for="store in dabingStores" :key="store.id" :label="store.name" :value="store.id" /></el-select></div><div><label>{{ t('lottery.drawTime') }}</label><el-input v-model="dabingTime" :placeholder="t('lottery.drawTimePlaceholder')" /></div></div>
        <div class="quick-entry-row"><el-select v-model="dabingPersonId" filterable clearable :placeholder="t('lottery.namePlaceholder')" class="person-select"><el-option v-for="person in filteredDabingPeople" :key="person.id" :label="`${person.name}${person.phoneLastFour ? ` · ${person.phoneLastFour}` : ''}`" :value="person.id" /></el-select><el-button class="save-button" type="primary" :loading="saving" :disabled="!dabingPersonId" @click="saveDabingRecord()">{{ t('lottery.saveRecord') }}</el-button></div>
        <div v-if="recentDabingChips.length" class="recent-row"><span class="recent-label">{{ t('lottery.recentPeople') }}</span><button v-for="person in recentDabingChips" :key="person.id" type="button" class="recent-chip" :disabled="saving" @click="saveDabingRecord(person.id)">{{ person.name }}</button></div>
        <p class="entry-note">{{ t('lottery.dabingEntryHint') }}</p>
      </section>
      <section class="card overview-card">
        <div class="section-heading">
          <div><span class="eyebrow">{{ t('lottery.overview') }}</span><h2 v-if="dabingView === 'today'">{{ dabingDate }} · {{ overviewDabingStoreName }}</h2><h2 v-else>{{ dabingRange[0] }} ~ {{ dabingRange[1] }} · {{ overviewDabingStoreName }}</h2></div>
          <div class="heading-actions"><el-button text :icon="showPhones ? Hide : View" @click="togglePhoneVisibility">{{ showPhones ? t('lottery.hidePhones') : t('lottery.showPhones') }}</el-button><el-button text :icon="Refresh" @click="refreshDabing" /></div>
        </div>
        <div class="view-toggle"><button type="button" :class="{ active: dabingView === 'today' }" @click="selectDabingView('today')">{{ t('lottery.viewToday') }}</button><button type="button" :class="{ active: dabingView === 'browse' }" @click="selectDabingView('browse')">{{ t('lottery.viewBrowse') }}</button></div>

        <template v-if="dabingView === 'today'">
          ...
          <div class="stat-grid"><div class="stat"><strong>{{ dabingRecords.length }}</strong><span>{{ t('lottery.winnerCount') }}</span></div><div class="stat"><strong>{{ dabingStores.length }}</strong><span>{{ t('lottery.storeCount') }}</span></div><div class="stat"><strong>{{ dabingRecords.filter((row) => row.createdByName).length }}</strong><span>{{ t('lottery.multiUserRecords') }}</span></div></div>
          <div v-if="dabingRecords.length" class="record-list"><article v-for="row in dabingRecords" :key="row.id" class="record-card"><div class="record-title"><strong>{{ row.personName }}</strong><span class="store-badge">{{ row.storeName }}</span></div><div class="record-detail"><span>{{ formatPhone(row.phone, row.phoneLastFour) }}</span><span v-if="row.mobileModel">{{ row.mobileModel }}</span><span v-if="row.drawTime">{{ row.drawTime }}</span></div><div class="record-footer"><span>{{ row.createdByName || t('lottery.importedRecord') }}</span><el-button text type="danger" :icon="Delete" @click="removeDabingRecord(row)" /></div></article></div>
          <div v-else class="empty-state">{{ t('lottery.emptyRecords') }}</div>
        </template>

        <template v-else>
          <div class="browse-controls"><el-date-picker v-model="dabingRange" type="daterange" value-format="YYYY-MM-DD" unlink-panels :start-placeholder="t('lottery.drawStartDate')" :end-placeholder="t('lottery.drawEndDate')" :shortcuts="rangeShortcuts" class="browse-range" /><el-input v-model="dabingBrowseKeyword" :placeholder="t('lottery.searchNamePlaceholder')" :prefix-icon="Search" clearable class="browse-search" /><el-select v-model="dabingOverviewStoreId" class="overview-store-select"><el-option :label="t('lottery.allStores')" value="all" /><el-option v-for="store in dabingStores" :key="store.id" :label="store.name" :value="store.id" /></el-select><el-button type="primary" plain :icon="Download" :loading="downloading" @click="exportDabing">{{ t('lottery.exportExcel') }}</el-button></div>
          <div class="stat-grid two"><div class="stat"><strong>{{ dabingBrowseRecords.length }}</strong><span>{{ t('lottery.winnerCount') }}</span></div><div class="stat"><strong>{{ dabingBrowseByDate.length }}</strong><span>{{ t('lottery.dayCount') }}</span></div></div>
          <div v-if="dabingBrowseByDate.length" class="browse-list"><section v-for="day in dabingBrowseByDate" :key="day.date" class="browse-day"><h3 class="browse-day-head">{{ day.date }}<span>{{ day.records.length }}</span></h3><article v-for="row in day.records" :key="row.id" class="record-card"><div class="record-title"><strong>{{ row.personName }}</strong><span class="store-badge">{{ row.storeName }}</span></div><div class="record-detail"><span>{{ formatPhone(row.phone, row.phoneLastFour) }}</span><span v-if="row.mobileModel">{{ row.mobileModel }}</span><span v-if="row.drawTime">{{ row.drawTime }}</span></div><div class="record-footer"><span>{{ row.createdByName || t('lottery.importedRecord') }}</span><el-button text type="danger" :icon="Delete" @click="removeDabingRecord(row)" /></div></article></section></div>
          <div v-else class="empty-state">{{ t('lottery.browseEmpty') }}</div>
        </template>
      </section>
    </template>

    <template v-else>
      <section class="entry-card card accent-kyoto">
        <div class="section-heading"><div><span class="eyebrow">{{ t('lottery.quickEntry') }}</span><h2>{{ t('lottery.kyotoEntryTitle') }}</h2></div><div class="heading-actions"><el-button text :icon="Plus" @click="openBatchDialog">{{ t('lottery.addBatch') }}</el-button><el-button text :icon="User" @click="openPersonDialog('kyoto')">{{ t('lottery.addPerson') }}</el-button></div></div>
        <div v-if="kyotoBatches.length" class="context-grid one-line"><div><label>{{ t('lottery.drawBatch') }}</label><el-select v-model="kyotoBatchId" :placeholder="t('lottery.selectBatch')"><el-option v-for="batch in kyotoBatches" :key="batch.id" :label="`${batch.displayLabel} · ${batch.publishDate}`" :value="batch.id" /></el-select></div></div>
        <div v-else class="empty-setup"><Calendar /><span>{{ t('lottery.noBatchHint') }}</span><el-button type="primary" plain @click="openBatchDialog">{{ t('lottery.addBatch') }}</el-button></div>
        <div v-if="kyotoBatches.length" class="quick-entry-row"><el-select v-model="kyotoPersonId" filterable clearable :placeholder="t('lottery.namePlaceholder')" class="person-select"><el-option v-for="person in filteredKyotoPeople" :key="person.id" :label="`${person.name}${person.phoneLastFour ? ` · ${person.phoneLastFour}` : ''}`" :value="person.id" /></el-select><el-button class="save-button" type="primary" :loading="saving" :disabled="!kyotoPersonId" @click="saveKyotoRecord()">{{ t('lottery.saveRecord') }}</el-button></div>
        <div v-if="kyotoBatches.length && recentKyotoChips.length" class="recent-row"><span class="recent-label">{{ t('lottery.recentPeople') }}</span><button v-for="person in recentKyotoChips" :key="person.id" type="button" class="recent-chip" :disabled="saving" @click="saveKyotoRecord(person.id)">{{ person.name }}</button></div>
        <p class="entry-note">{{ t('lottery.kyotoEntryHint') }}</p>
      </section>
      <section class="card overview-card kyoto-overview">
        <div class="section-heading">
          <div><span class="eyebrow">{{ t('lottery.overview') }}</span><h2 v-if="kyotoView === 'batch'">{{ selectedKyotoBatch?.displayLabel || '—' }}</h2><h2 v-else>{{ kyotoRange[0] }} ~ {{ kyotoRange[1] }}</h2><p class="section-subtitle">{{ kyotoView === 'batch' ? (selectedKyotoBatch?.publishDate || '—') : t('lottery.viewAll') }} · {{ t('lottery.groupedByName') }}</p></div>
          <div class="heading-actions"><el-button text :icon="showPhones ? Hide : View" @click="togglePhoneVisibility">{{ showPhones ? t('lottery.hidePhones') : t('lottery.showPhones') }}</el-button><el-button text :icon="Refresh" @click="refreshKyoto" /></div>
        </div>
        <div class="view-toggle"><button type="button" :class="{ active: kyotoView === 'batch' }" @click="selectKyotoView('batch')">{{ t('lottery.viewThisBatch') }}</button><button type="button" :class="{ active: kyotoView === 'all' }" @click="selectKyotoView('all')">{{ t('lottery.viewAll') }}</button></div>

        <div v-if="kyotoView === 'all'" class="browse-controls"><el-date-picker v-model="kyotoRange" type="daterange" value-format="YYYY-MM-DD" unlink-panels :start-placeholder="t('lottery.publishDate')" :end-placeholder="t('lottery.publishDate')" :shortcuts="rangeShortcuts" class="browse-range" /><el-input v-model="kyotoAllKeyword" :placeholder="t('lottery.searchNamePlaceholder')" :prefix-icon="Search" clearable class="browse-search" /></div>
        <div class="filter-row"><span class="privacy-note">{{ showPhones ? t('lottery.phoneVisible') : t('lottery.phoneHidden') }}</span><el-button type="primary" plain :icon="Download" :loading="downloading" :disabled="kyotoView === 'batch' && !selectedKyotoBatch" @click="exportKyoto">{{ t('lottery.exportExcel') }}</el-button></div>
        <div class="stat-grid kyoto-stats"><div class="stat highlight"><strong>{{ groupedKyotoRecords.length }}</strong><span>{{ t('lottery.groupedWinnerCount') }}</span></div><div class="stat"><strong>{{ kyotoPhoneTotal }}</strong><span>{{ t('lottery.phoneCount') }}</span></div><div class="stat"><strong>{{ kyotoActiveRecords.length }}</strong><span>{{ t('lottery.recordCount') }}</span></div></div>
        <div v-if="groupedKyotoRecords.length" class="winner-list"><article v-for="group in groupedKyotoRecords" :key="group.key" class="winner-card" :class="{ expanded: expandedKyotoNames[group.key] }"><button type="button" class="winner-summary" @click="toggleKyotoGroup(group.key)"><span class="name-avatar">{{ group.name.slice(0, 1) }}</span><span class="winner-copy"><strong>{{ group.name }}</strong><small>{{ group.phoneCount }} {{ t('lottery.phoneCount') }} · {{ group.records.length }} {{ t('lottery.recordCount') }}</small></span><span class="expand-icon"><ArrowUp v-if="expandedKyotoNames[group.key]" /><ArrowDown v-else /></span></button><div v-if="expandedKyotoNames[group.key]" class="phone-list"><div v-for="row in group.records" :key="row.id" class="phone-row"><div><strong>{{ formatPhone(row.phone, row.phoneLastFour) }}</strong><small>{{ kyotoView === 'all' ? row.batchLabel : (row.createdByName || t('lottery.importedRecord')) }}</small></div><el-button text type="danger" :icon="Delete" @click="removeKyotoRecord(row)" /></div></div></article></div>
        <div v-else class="empty-state">{{ t('lottery.emptyRecords') }}</div>
      </section>
    </template>
...
.recent-row { display: flex; flex-wrap: wrap; align-items: center; gap: 7px; margin-top: 12px; }.recent-label { color: var(--text-tertiary); font-size: 11px; font-weight: 700; }.recent-chip { border: 1px solid var(--border); border-radius: 999px; padding: 6px 13px; min-height: 32px; background: var(--surface-alt); color: var(--text-primary); font-size: 13px; cursor: pointer; transition: border-color .14s ease, background .14s ease; }.recent-chip:hover:not(:disabled) { border-color: var(--accent); background: var(--surface); }.recent-chip:disabled { opacity: .5; cursor: default; }
.view-toggle { display: inline-flex; gap: 2px; padding: 3px; margin-bottom: 14px; background: var(--surface-alt); border-radius: 11px; }.view-toggle button { border: 0; background: transparent; color: var(--text-secondary); font-size: 13px; padding: 6px 16px; min-height: 34px; border-radius: 9px; cursor: pointer; }.view-toggle button.active { background: var(--surface); color: var(--text-primary); font-weight: 700; box-shadow: var(--shadow-soft); }
.browse-controls { display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 14px; }.browse-range { width: 260px; }.browse-search { flex: 1; min-width: 160px; }.browse-controls .overview-store-select { width: 130px; }.browse-controls .el-button { margin-left: auto; }
.browse-list { display: flex; flex-direction: column; gap: 14px; }.browse-day-head { display: flex; align-items: center; gap: 8px; margin: 0 0 8px; color: var(--text-secondary); font-size: 13px; font-weight: 700; }.browse-day-head span { display: inline-grid; place-items: center; min-width: 20px; height: 20px; padding: 0 5px; border-radius: 999px; background: var(--surface-alt); color: var(--text-tertiary); font-size: 11px; }.browse-day { display: flex; flex-direction: column; gap: 9px; }
.overview-store-select { width: 140px; }.stat-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; margin-bottom: 16px; }.stat-grid.two { grid-template-columns: repeat(2, 1fr); }.stat { min-width: 0; padding: 13px 15px; background: var(--surface-alt); border-radius: 12px; }.stat.highlight { background: #fff0f5; }.stat strong { display: block; color: var(--text-primary); font-size: 24px; line-height: 1.1; }.stat span { display: block; color: var(--text-secondary); font-size: 11px; margin-top: 6px; }.record-list, .winner-list { display: flex; flex-direction: column; gap: 9px; }.record-card, .winner-card { border: 1px solid var(--border); border-radius: 13px; overflow: hidden; background: var(--surface); }.record-card { padding: 13px 14px 8px; }.record-title { display: flex; align-items: center; gap: 8px; }.record-title strong { color: var(--text-primary); font-size: 15px; }.store-badge { color: #2367d1; background: #eef5ff; border-radius: 999px; padding: 3px 8px; font-size: 11px; }.record-detail { display: flex; flex-wrap: wrap; gap: 6px 14px; color: var(--text-secondary); font-size: 12px; margin-top: 10px; }.record-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 5px; color: var(--text-tertiary); font-size: 11px; }
