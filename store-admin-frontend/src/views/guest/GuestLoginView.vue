...
import { guestLogin, recoverCard } from '@/api/guest'
...
// 'pin'  — phone + 6-digit PIN, regain full use of the card on a new device.
...
const canSubmit = computed(() => {
  if (submitting.value || !phoneOk.value) return false
  return mode.value === 'pin' ? /^\d{6}$/.test(form.pin) : !!birthdayMd.value
})
...
async function submit() {
  ...
  try {
    if (mode.value === 'pin') {
      await recoverCard(form.phone.trim(), form.pin)
      router.replace({ name: 'guest-card' })
    } else {
      // Verify the pair resolves before navigating; the card page fetches
      // the snapshot again from router state.
      await guestLogin(form.phone.trim(), birthdayMd.value)
      router.push({
        name: 'guest-card',
        query: { readonly: '1' },
        state: { phone: form.phone.trim(), birthdayMd: birthdayMd.value },
      })
    }
  } catch (err) {
